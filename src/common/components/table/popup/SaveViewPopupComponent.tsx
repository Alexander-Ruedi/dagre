...
interface SaveViewInterface {
  name: string;
}

export const SaveViewPopupComponent = (props: DialogProps) => {
  ...
  const createSchema = z.object({
    name: z
      ...
      .refine((newName) => props.forbiddenNames && props.forbiddenNames.indexOf(newName) === -1, {
        message: "This view already exists.",
      }),
  });
  const SaveViewSchema: z.ZodType<SaveViewInterface> = createSchema;

  const {
    register,
    formState: { errors },
    handleSubmit,
  } = useForm<SaveViewInterface>({
              ...
              <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-sm sm:p-6">
                <div>
                  <div className="mt-3 text-center sm:mt-5">
                    ...
                    <div className="mt-2">
                      <p className="text-sm text-gray-500">Please enter a name for the view.</p>
                    </div>
                    <div className="col-span-3 mt-2">
                      <div className="mt-1">
                        <input type="text" className={defaultInputStyle} {...register("name")} data-testid="viewName" />
                      </div>
                ...
                <div className="w-full mt-8 flex">
                  ...
                  <LoadingSpinnerDialogButtonComponent
                    onClick={handleSubmit((saveView) => {
                      setOpen(false);
                      props.onConfirm(saveView.name);
                      notifySuccess("View", EventType.save);
                    })}
                    label="Save"
                    isLoading={false}
                  />
